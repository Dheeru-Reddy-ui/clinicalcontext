...
- **fusion** ([fusion.py](backend/app/retrieval/fusion.py)) — Reciprocal Rank Fusion (`k=60`), configurable dense/lexical weights. **Exact matches on a rare term always reach the reranker**: a passage carrying a query term found in at most 10 passages (a unique token, a rare drug or disease name) is kept through fusion, however the two rankings placed it — `ts_rank_cd` does not weigh rarity, and the approximate vector index can miss a vector (or a paper filed moments ago has none yet), so RRF used to drop the one passage that named the rare term. The terms are counted live, only for words `lexeme_stats` doesn't already know to be common.
